...
Post.reusablePostQuery = function (uniqueOperations, visitorId, finalOperations = []) {
  return new Promise(async function (resolve, reject) {
    let aggOperations = uniqueOperations.concat([{ $lookup: { from: "users", localField: "author", foreignField: "_id", as: "authorDocument" } }, { $project: { title: 1, body: 1, createdDate: 1, authorId: "$author", author: { $arrayElemAt: ["$authorDocument", 0] } } }])
...
Post.search = function (term) {
  return new Promise(async (resolve, reject) => {
    if (typeof term == "string") {
      let posts = await Post.reusablePostQuery([{ $match: { $text: { $search: term } } }, { $sort: { score: { $meta: "textScore" } } }])
      resolve(posts)
    } else {
      reject()
    }
